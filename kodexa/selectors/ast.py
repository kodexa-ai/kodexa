...
class PipelineExpression(object):
    ...
    def resolve(self, content_node: ContentNode, variables, context: SelectorContext):
        ...
        for node in left_nodes:
            result_nodes = result_nodes + self.right.resolve(node, variables, context)
        return result_nodes
            ...
            for predicate in self.predicates:
                if isinstance(predicate, int):
                    if predicate == axis_node.index:
                        match = True
                elif not predicate.resolve(axis_node, variables, context):
                    match = False

            match = match and self.node_test.test(axis_node, variables, context)
