...
def test_date_validation_rules():
    ...
    taxonomy = Taxonomy(type="taxonomy", name="DateTesting", slug="date-testing", version="1.0.0")
    invoice = Taxon(path="invoice", name="invoice", group=True)
    invoice_date = Taxon(path="invoice/date", name="date", taxon_type="DATE")
    due_date = Taxon(path="invoice/duedate", name="duedate", taxon_type="STRING")
    birth_date = Taxon(path="invoice/birthdate", name="birthdate", taxon_type="DATE")
    ...
    range_validation = TaxonValidation(
        ...
        rule_formula="isBlank({date}) || (isafterdate({date}, datemath(\"today - 30 days\")) && isbeforedate({date}, datemath(\"today + 30 days\")))"
    )
    
    due_date_validation = TaxonValidation(
        name="DueDateAfterInvoice",
        description="Due date must be after invoice date",
        rule_formula="isBlank({duedate}) || isBlank({date}) || isBeforeDate({duedate}, datemath({date}, \"days\", 101))"
    )
    
    birth_date_validation = TaxonValidation(
        name="BirthDatePast",
        description="Birth date must be in the past",
        rule_formula="isBlank({birthdate}) || (isdate({birthdate}) && isbeforedate({birthdate}, \"today\"))"
    )
    ...
    document_validations = [
        DocumentTaxonValidation(taxonomy_ref="test/date-taxonomy:1.0.0", taxon_path="invoice/date", validation=required_validation),
        DocumentTaxonValidation(taxonomy_ref="test/date-taxonomy:1.0.0", taxon_path="invoice/date", validation=format_validation),
        DocumentTaxonValidation(taxonomy_ref="test/date-taxonomy:1.0.0", taxon_path="invoice/date", validation=range_validation),
        DocumentTaxonValidation(taxonomy_ref="test/date-taxonomy:1.0.0", taxon_path="invoice/duedate", validation=due_date_validation),
        DocumentTaxonValidation(taxonomy_ref="test/date-taxonomy:1.0.0", taxon_path="invoice/birthdate", validation=birth_date_validation),
        DocumentTaxonValidation(taxonomy_ref="test/date-taxonomy:1.0.0", taxon_path="invoice/date", validation=leap_year_validation)
    ]
    ...
    date_validations = [v for v in validations if v.taxon_path == "invoice/date"]
    assert len(date_validations) == 4
    
    due_date_validations = [v for v in validations if v.taxon_path == "invoice/duedate"]
    assert len(due_date_validations) == 1
    
    birth_date_validations = [v for v in validations if v.taxon_path == "invoice/birthdate"]
    assert len(birth_date_validations) == 1
    
    validation_names = [v.validation.name for v in validations]
    expected_names = ["DateRequired", "ValidDateFormat", "DateRange", "DueDateAfterInvoice", "BirthDatePast", "LeapYearValidation"]
    ...
    assert any("isbeforedate" in v.validation.rule_formula for v in validations)
