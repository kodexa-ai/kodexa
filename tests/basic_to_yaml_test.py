from kodexa import Pipeline, RemoteStep
...
def test_to_yaml():
    ...
    pipeline.add_step(RemoteStep(ref='kodexa/pdf-parser',
    ...
    transactions_header_re = r'^Date\s+Description.*\s+Amount$'

    # Extract Other Deposits
    other_deposits_table_tag_name = "Other Deposits"
    other_deposits_re = '^Other Deposits$'
    total_other_deposits_re = r'^Total Other Deposits.*\d{2}$'
    balance_re = '^BALANCE YOUR ACCOUNT$'
    pipeline.add_step(RemoteStep(ref='kodexa/pattern-table-tagger',
    ...
    card_withdrawals_re = r'^Card Withdrawals$'
    subtotal_card_withdrawals_re = r'^Card \d{4} Withdrawals Subtotal.*\d{2}.$'
    total_card_withdrawals_re = r'^Total Card Withdrawals.*\d{2}.$'
    pipeline.add_step(RemoteStep(ref='kodexa/pattern-table-tagger',
                                 options={"col_space_multiplier": col_space_multiplier,
    ...
    other_withdrawals_re = r'^Other Withdrawals$'
    total_other_withdrawals_re = r'^Total Other Withdrawals.*\d{2}.$'
    pipeline.add_step(RemoteStep(ref='kodexa/pattern-table-tagger',
                                 options={"col_space_multiplier": col_space_multiplier,
    ...
    check_transactions_re = r'^Check Date .* Ref Number Amount$'
    checks_re = r'^Checks Presented Conventionally$'
    checks_paid_re = r'.*Conventional Checks Paid.*\d{2}.$'
